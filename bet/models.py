...
class Bankroll(models.Model):
    ...
    def _apply_movement(
        self, amount: Decimal | str, status: int, note: str | None = None
    ):
        ...
        if self.pk is None:
            raise ValueError(
                "A banca precisa ser salva antes de registrar movimentações."
            )
...
class BankrollHistory(models.Model):
    ...
    bankroll = models.ForeignKey(
        "Bankroll", on_delete=models.CASCADE, related_name="history"
    )
...
class Bet(models.Model):
    ...
    def register_bet(self):
        ...
        with transaction.atomic():
            ...
            bankroll.withdraw(
                self.stake, note=f"Aposta em {self.match} ({self.market})"
            )
...
class AllowedLeague(models.Model):
    ...
    active = models.BooleanField(
        default=True, help_text="Se desmarcado, a liga será ignorada no scraping."
    )
